...
def _render_availability(context: dict) -> str:
    ...
    """
    panel = _availability_dashboard(context) if context["view"] == "dashboard" else _availability_form(context)
    body = f"""
    <section class=\"content-main\">
...
def _availability_dashboard(context: dict) -> str:
    ...
    for time_label in context['time_labels']:
        ...
        for day in WEEKDAY_ORDER:
            ...
            cells.append(f'<div class="calendar-cell {class_name}">{escape(priority)}</div>')
    return f"""
...
def _schedule_columns(slots: list[dict]) -> str:
    markup = []
    for slot in slots:
        names = ''.join(f'<li>{escape(item["name"])}</li>' for item in slot['assigned'])
...
def _side_nav(user: dict, active: str, role: str) -> str:
    if role == 'admin':
        items = [("Auto Schedule", f'/admin?user={user["id"]}&role=admin', True)]
    else:
        items = [
...
def _options(options: list[str], current: str, blank_label: str = 'Select') -> str:
    ...
    for option in options:
        ...
        html.append(f'<option value="{escape(option)}"{selected}>{escape(option)}</option>')
    return ''.join(html)
